"""Madison Avenue Couture scraper — Shopify storefront fallback using products.json."""
import json
import re
...
class MadisonAvenueCoutureScraper(BaseScraper):
    platform = Platform.MADISON_AVENUE_COUTURE
    base_url = "https://madisonavenuecouture.com"

    def _parse_condition(self, body_html: str, tags: list[str]) -> str:
        plain = re.sub(r"<[^>]+>", " ", body_html or "")
        for source in [plain, " ".join(tags)]:
            lowered = source.lower()
            if "new" in lowered or "store fresh" in lowered:
                return "pristine"
    ...
    async def scrape(self) -> int:
        ...
        while True:
            ...
            if not data or not data.get("products"):
                if page == 1:
                    self.fail_scrape(error="[Madison Avenue Couture] No products found on page 1 — feed unavailable or blocked")
                break
            ...
            for product in data["products"]:
                try:
                    ...
                    print(f"[Madison Avenue Couture] Error processing product: {exc}")
        ...
        if total_found == 0:
            self.fail_scrape(error="[Madison Avenue Couture] Parsed pages but extracted zero discounted listings")

        self.log_scrape(True, total_found, total_new, total_updated)
        print(f"[Madison Avenue Couture] Done: {total_found} found, {total_new} new, {total_updated} updated")
